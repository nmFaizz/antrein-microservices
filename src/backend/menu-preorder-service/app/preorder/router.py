...
import httpx
from app.auth.dependencies import bearer_scheme, get_current_user, require_admin
from app.core.remote import extract_message
from app.core.response import APIResponse, ok
from app.db.config import settings
from app.db.database import get_session
from app.menu.models import Menu
from app.preorder.models import (
    Preorder,
    PreorderCreate,
    PreorderItem,
    PreorderItemCreate,
...
def list_preorders(
    session: Session = Depends(get_session), current_user=Depends(require_admin)
):
    """
...
def list_user_preorders(
    ...
    preorders = session.exec(
        select(Preorder).where(Preorder.user_id == current_user["user_id"])
    ).all()
    return ok(preorders, "Preorders retrieved successfully")
...
def validate_and_calculate_items(
    items: List[PreorderItemCreate], session: Session
) -> tuple[float, List[PreorderItem]]:
    ...
    for item in items:
        ...
        db_item = PreorderItem(
            menu_item_id=item.menu_item_id, quantity=item.quantity, subtotal=subtotal
        )
...
def register_queue_for_preorder(user_id: UUID, preorder_id: UUID) -> dict:
    ...
    try:
        response = httpx.post(
            ...
            json={"customer_id": str(user_id), "preorder_id": str(preorder_id)},
            timeout=10.0,
        )
        if response.status_code == 201:
            ...
            raise HTTPException(
                ...
                detail=extract_message(response, "Failed to create queue"),
            )
...
@router.post(
    "/",
    response_model=APIResponse[PreorderResponse],
...
def create_preorder(
    ...
    total_price, preorder_items = validate_and_calculate_items(
        preorder_in.items, session
    )
...
def cancel_preorder(
    ...
    Cancel a preorder.
    """
    statement = select(Preorder).where(
        Preorder.id == preorder_id,
        Preorder.user_id == current_user["user_id"],
