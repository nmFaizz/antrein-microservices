              ...
              >
                ...
                <p className="mt-2 line-clamp-1 text-sm text-muted-foreground">
                  {order.items
                    .map(
                      (item) =>
                        `${item.quantity}x ${item.menu?.name ?? `Item #${item.menu_item_id.slice(0, 6)}`}`,
                    )
                    .join(", ")}
                </p>
          ...
          <div className="flex flex-col gap-4">
            ...
            {selected.queue?.queue_number != null &&
              ...
              (() => {
                ...
                if (isPending) {
                  const badge = queueBadge[q.status] ?? {
                    label: q.status,
                        ...
                        {q.estimated_time && (
                          <div className="mt-1 w-full rounded-md bg-muted p-2">
                            ...
                            <p className="text-lg font-bold">
                              {formatEstimatedTime(q.estimated_time)}
                            </p>
                          </div>
                        )}
                      </CardContent>
