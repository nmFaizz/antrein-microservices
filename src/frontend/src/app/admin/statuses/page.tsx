...
import {
  ...
  useStatuses,
  useUpdateStatus,
} from "@/features/queue-status/queries";
...
export default function StatusesPage() {
  ...
  const saveError = createStatus.error?.message ?? updateStatus.error?.message;
  ...
  function save(data: QueueStatusValues) {
    ...
    const payload = { name, color: data.color };
    if (formMode === "add") {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {deleteStatus.error && (
        <p className="text-sm text-destructive">{deleteStatus.error.message}</p>
      )}
